/* CORECONF <-> CBOR serialization routines built on top of zcbor. */
...
bool _parse_map(zcbor_state_t *state, CoreconfValueT *coreconfValue, unsigned indent) {
    ...
    while (!zcbor_array_at_end(state)) {
        if (loopCount > CORECONF_MAX_LOOP) return false;  // Infinite-loop protection
